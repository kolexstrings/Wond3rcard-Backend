...
const fontsStorage = multer.diskStorage({
  ...
  filename: (req, file, cb) => {
    try {
      ...
      cb(
        new HttpException(400, "Invalid file", "Failed to generate font name"),
        null
      );
    }
  },
...
const fontsFilter = (req: any, file: any, cb: any) => {
  ...
  if (
    ...
    return cb(
      new HttpException(400, "Invalid font", "Only font files are allowed"),
      false
    );
  }
};
    ...
    if (!file.mimetype.startsWith("video/")) {
      return cb(
        new HttpException(
          400,
          "Invalid file type",
          "Only video files are allowed for cardVideo!"
        ),
        false
      );
    }
    ...
    if (!file.mimetype.startsWith("image/")) {
      return cb(
        new HttpException(
          400,
          "Invalid file type",
          "Only image files are allowed for cardPhoto and cardCoverPhoto!"
        ),
        false
      );
    }
...
// Middleware for uploading a single social media image
export const uploadCatelogueMiddleware =
  uploadCatelogueImage.single("cateloguePhoto");
